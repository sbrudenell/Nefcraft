...
new BeamSprite,HaloSprite,GlowSprite; //wards
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if(num==60)
	{
		...
		thisRaceID=War3_CreateNewRaceT("shadow_hunter");

		#if defined SOURCECRAFT
		    SetRaceAttributes(thisRaceID, .short="shadow_hunter",
				      .faction=OrcishHoard, .type=Biological);
		#endif
...
public OnMapStart()
{
	...
	HaloSprite=PrecacheModel("effects/tp_eyefx/tpeye3.vmt");
	GlowSprite=PrecacheModel("materials/sprites/greenglow1.vmt");
...
public WardEffectAndDamage(owner,wardindex)
{
	...
	TE_SetupBeamPoints(start_pos,end_pos,BeamSprite,HaloSprite,0,GetRandomInt(30,100),0.17,10.0,10.0,0,0.0,beamcolor,10);
	TE_SendToAll();

	TE_SetupBeamRingPoint(start_pos, 10.0, float(WARDRADIUS), BeamSprite,HaloSprite, 0, 15, 1.0, 15.0, 0.0, beamcolor, 10, 0);
	TE_SendToAll();

	TE_SetupGlowSprite(end_pos, GlowSprite, 1.0, 1.0, 50);
	TE_SendToAll();
