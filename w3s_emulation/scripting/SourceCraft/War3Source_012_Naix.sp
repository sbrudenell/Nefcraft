...
new Float:HPPercentHealPerKill[5] = { 0.0,0.15,  0.25,  0.35,  0.40 }; //SKILL_INFEST settings
...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if(num==120)
	{
		...
		ULT_RAGE = War3_AddRaceSkillT(thisRaceID, "Rage", true);
		    ...
		    SetRaceAttributes(thisRaceID, .faction=UndeadScourge, .type=Undead);
		#endif
	...
	if(ValidPlayer(victim)&&ValidPlayer(attacker)&&IsOurRace(attacker)){
		...
		if(level>0&&!Hexed(attacker,false)&&W3ChanceModifier(attacker)){
			if(!W3HasImmunity(victim,Immunity_Skills)){	
				if (GetRandomFloat(0.0,1.0)<=W3ChanceModifier(attacker)){
