...
#tryinclude <tf2_objects>
    ...
    stock const TF2_MaxSentryRockets[] = {  20,   0,   0,  20,  64 };
    stock const TF2_SentryHealth[]     = { 100, 150, 180, 216, 511 };

    stock TFExtObjectType:TF2_GetExtObjectTypeFromClass(entity, bool:specific=false)
    {
        decl String:class[32];
        if (GetEdictClassname(entity,class,sizeof(class)))
        {
            if (StrEqual(class, TF2_ObjectClassNames[TFObject_Sentry]))
            {
                return (specific) ? (GetEntProp(entity, Prop_Send, "m_bMiniBuilding")
                                    ? TFExtObject_MiniSentry : TFExtObject_Sentry)
                                  : TFExtObject_Sentrygun;
            }
            else if (StrEqual(class, TF2_ObjectClassNames[TFExtObject_Dispenser]))
                return TFExtObject_Dispenser;
            else if (StrEqual(class, TF2_ObjectClassNames[TFExtObject_Teleporter]))
            {
                return (specific) ? (TF2_GetObjectMode(entity) == TFObjectMode_Exit)
                                    ? TFExtObject_TeleporterExit : TFExtObject_TeleporterEntry
                                  : TFExtObject_Teleporter;
            }
            else if (StrEqual(class, TF2_ObjectClassNames[TFObjectType_Sapper]))
                return TFExtObject_Sapper;
            else
                ...
                else
                {
                    if (permissions > 0) // Hack to find native bits
                    {
                        switch (TF2_GetPlayerClass(i))
                        {
                            case TFClass_Spy:
                            {
                                new pcond = TF2_GetPlayerConditionFlags(i);
...
RemoteControl(client, TFExtObjectType:type)
{
    ...
    if (target > 0) 
    {
        type = TF2_GetExtObjectTypeFromClass(target, true);
    ...
    if (type == TFExtObject_Unknown)
    {
        ...
        {
            CancelMenu(menu);
            control(client, target, TF2_GetExtObjectTypeFromClass(target));
            return;
        }
        else if (sum == 0)
        {
                    ...
                    bool:drop=true, bool:check=true, Float:objectPosition[3]={0.0})
{
    ...
    if (res == Plugin_Continue)
    {
        ...
        switch (type)
        {
            ...
            case TFExtObject_TeleporterEntry:
            {
                objectid = BuildTeleporterEntry(client, pos, angles, iLevel, disable,
                                                iHealth, iMaxHealth, flPercentage);
            }
        ...
        if (g_RemoteType[index] == type)
        {
            if (objectid <= 0)
            {
                for (new i=MaxClients+1;i<g_iMaxEntities;i++)
                {
                    ...
                    {
                        if (TF2_GetExtObjectTypeFromClass(i) == type)
                        {
