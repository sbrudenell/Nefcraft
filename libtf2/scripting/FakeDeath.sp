...
public OnMapStart()
{
	...
	for (new i = 0; i < sizeof(GenericDeathSound); i++)
	    PrecacheSound(GenericDeathSound[i], true);
}
...
{	
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			// アタック1
			if (TF2_IsPlayerCloaked(i) && (GetClientButtons(i) & IN_ATTACK))
			{
				// 連続押し防止？
...
stock bool:TF2_IsPlayerCloaked(client)
{
    ...
    return pcond >= 0 ? ((pcond & TF_CONDFLAG_CLOAKED) != 0) : false;
}

stock bool:TF2_IsPlayerOnFire(client)
{
    new pcond = TF2_GetPlayerConditionFlags(client);
    return pcond >= 0 ? ((pcond & TF_CONDFLAG_ONFIRE) != 0) : false;
}
...
//				PrintToChatAll("%d", GetEntData(client, offset));
				...
				if (class == TFClass_Spy)	
				{
					EmitSoundToAll(DeathSound[GetRandomInt(0,sizeof(DeathSound)-1)], FakeBody, _, _, _, 1.0);
				...
				{
					EmitSoundToAll(GenericDeathSound[GetRandomInt(0,sizeof(GenericDeathSound)-1)], FakeBody, _, _, _, 1.0);
				}
