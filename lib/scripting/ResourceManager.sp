...
public OnConfigsExecuted()
{
    ...
    LogToGame("%d/%d Sounds loaded by Configs Executed time",
...
public OnMapEnd()
{
    LogToGame("%d/%d Decals loaded by Map End time",
              g_iDecalCount, g_decalTrie ? GetTrieSize(g_decalTrie) : 0);

    LogToGame("%d/%d Models loaded by Map End time",
              g_iModelCount, g_modelTrie ? GetTrieSize(g_modelTrie) : 0);

    new numSounds = g_soundTrie ? GetTrieSize(g_soundTrie) : 0;
    LogToGame("%d/%d Sounds loaded by Map End time", g_iSoundCount, numSounds);
...
public Native_SetupModel(Handle:plugin,numParams)
{
    ...
    if (index < 1 && (g_iModelCount <= g_iModelThreshold ||
...
public Native_SetupDecal(Handle:plugin,numParams)
{
    ...
    if (index < 1 && (g_iDecalCount <= g_iDecalThreshold ||
