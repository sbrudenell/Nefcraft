...
// Sounds
new String:grabberHitWav[PLATFORM_MAX_PATH] = "sc/zluhit00.mp3"; // "weapons/crossbow/bolt_skewer1.wav";
new String:pullerWav[PLATFORM_MAX_PATH] = "sc/intonydus.mp3"; // "weapons/crowwbow/hitbod2.wav";
new String:deniedWav[PLATFORM_MAX_PATH] = "sc/buzz.wav"; // "buttons/combine_button_locked.wav";
new String:errorWav[PLATFORM_MAX_PATH] = "sc/perror.mp3"; // "player/suit_denydevice.wav";
new String:seekingWav[PLATFORM_MAX_PATH] = "sc/ropeshoot2.wav"; // "weapons/crossbow/bolt_fly4.wav"; // "weapons/tripwire/ropeshoot.wav";
...
bool:SetupSound(const String:wav[], bool:preload=false)
{
    if (wav[0])
    {
    ...
    if (g_bNativeOverride || GetConVarBool(cvarHookEnable))
    {
        if (client>0)
        {
                ...
                IsClientInGame(client) && IsPlayerAlive(client))
            {
                ...
                {
                    if (GetConVarBool(cvarHookNoFlag) && TF2_HasTheFlag(client))
                    {
...
public Action:Hooking(Handle:timer,any:userid)
{
    ...
    if (index > 0 && gStatus[index][ACTION_HOOK] && !gGrabbed[index] &&
...
{
    new index = GetClientOfUserId(userid);
    if (index > 0 && !gGrabbed[index] && gStatus[index][ACTION_GRAB] &&
        IsClientInGame(index) && IsPlayerAlive(index))
    {
        ...
        if (target > 0 && target <= MaxClients && IsClientInGame(target))
        {
            ...
            if (bValid)
            {
                ...
                if (g_bIsTF2)
                {
                    new pcond = TF2_GetPlayerConditionFlags(target);
                    if (TF2_IsDazed(pcond) || TF2_IsBonked(pcond) || TF2_IsCritCola(pcond) || 
                        TF2_IsTaunting(pcond) || TF2_IsCharging(pcond)||
                        GetEntProp(target, Prop_Send, "m_iStunFlags") != 0 ||
...
public Action:Grabbing(Handle:timer,any:userid)
{
    ...
    {
        if (gStatus[index][ACTION_GRAB] && !gGrabbed[index] && IsPlayerAlive(index))
        {
                ...
                IsClientInGame(target) && IsPlayerAlive(target))
            {
                ...
                if (g_bIsTF2)
                {
                    new pcond = TF2_GetPlayerConditionFlags(index);
                    if (TF2_IsDazed(pcond) || TF2_IsBonked(pcond) || TF2_IsCritCola(pcond) || 
                        TF2_IsTaunting(pcond) || TF2_IsCharging(pcond)||
                        GetEntProp(index, Prop_Send, "m_iStunFlags") != 0 ||
                        TF2_HasTheFlag(index))
                    {
                        Action_Drop(index);
                        return Plugin_Stop;
                    }
